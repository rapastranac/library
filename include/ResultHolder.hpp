...
namespace library {
	...
	protected:
		BranchHandler* branchHandler = nullptr;// = library::BranchHandler();
	...
	public:
		ResultHolder(library::BranchHandler* handler)
		{
		...
		~ResultHolder()
		{
			/*To ensure that if this holder dies, then it should dissappear from
				children's parent to avoid exeptions*/
			if (!children.empty())
			{
		...
		ResultHolder(library::BranchHandler* handler, ResultHolder* parent)
		{
			this->branchHandler = handler;
			this->id = branchHandler->getUniqueId();
			this->isPushed = false;
		...
		ResultHolder(ResultHolder&& src) noexcept
		{
			...
			//this->expectedFut = std::move(src.expectedFut); //Unique_ptr check it out
		...
		{
			if (branchHandler->appliedStrategy != 1)
			{
